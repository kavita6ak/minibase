...
class BMDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1 () {
	...
	try
	{
...

 	{
		  ...
		  if(replacealgo[i].equals("LRUK"))
		  {
			  SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, replacealgo[i],5);
		  }
		  else
		  {
  ...
  protected boolean test2 () {
	...
	try
	{
	...
	catch (Exception e)
 	{
		  ...
		  if(replacealgo[i].equals("LRUK"))
		  {
			  SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, replacealgo[i],1);
		  }
		  else
		  {
			  SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, replacealgo[i]);
    ...
    for ( pid.pid=firstPid.pid+1, lastPid.pid=firstPid.pid+numPages-1;
      ...
      try {
    	 // System.out.println("PINPAGE");
        SystemDefs.JavabaseBM.pinPage( pid, pg, /*emptyPage:*/ true );
       ///.out.println("PINPAGE2");
      }
      catch (Exception e) { 
	status = FAIL;
	System.err.print("*** Could not pin new page "+pid.pid+"\n");
  ...
  protected boolean test4 () {
	    ...
	    PageId firstPid = new PageId();
	    boolean status = OK;
	    SystemDefs sysdef = new SystemDefs( dbpath, NUMBUF+20, NUMBUF, "LRUK",5);
		...
		int pagenumber=-1;
	        ...
	        try
	        {
	        	...
	        	{
	        		SystemDefs.JavabaseBM.unpinPage( SystemDefs.JavabaseBM.newPage(pg,1), true );
	        	}
	        }
	        catch (Exception e) 
	        { 
